...
{
	vec2 x = floor(uv - vec2(2., 2.));
	vec2 y = fract(uv - vec2(2., 2.));
...
void main()
{
	...
	vec3 color = vec3(0.0);
	float noise = noise(st * 5.0 + u_time);
	float fractal = fbm(st * 5.0 + u_time, .5);
	color = vec3(mix(noise, fractal, abs(sin(.5))));
